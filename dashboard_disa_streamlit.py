...
<<<<<<< HEAD
        ...
        with open(CSV_FILE,"a",newline="",encoding="utf-8") as f:
            csv.writer(f).writerow([now,mode,soc,voltage,current,power,temp,humid,lux,
                rels.get("r1_critical",False),rels.get("r2_noncrit",False),a_count])

        st.session_state.last = {
            "ts":now,"mode":mode,"soc":soc,
            "voltage":voltage,"current":current,"power":power,
            "temp":temp,"humid":humid,"lux":lux,
...
<<<<<<< HEAD
    ...
    process_data({"device":"DISA-SIM","version":"4.2","ts":int(t*1000),
        ...
        "battery":{"soc_pct":round(soc,1),"voltage_v":round(11+soc/45,2),
                   ...
                   "power_w":round(random.uniform(18,42),1),"system_v":12,"ina219_ok":True},
        "env":{"temp_c":round(32+random.uniform(-2,5),1),"humid_pct":round(65+random.uniform(-5,10),1),
               "lux":round(random.uniform(2000,5000),0),"solar_w":round(random.uniform(50,100),1),
               "dht_ok":True,"bh1750_ok":True},
        "relays":{"r1_critical":soc>40,"r2_noncrit":soc>50},
...
<<<<<<< HEAD
        ...
        pred = last.get("predictif", {})
        if pred:
            st.markdown("#### 🧠 Module Prédictif Adaptatif")
            pred_mode_val = pred.get("pred_mode", "—")
            pred_color    = {"NORMAL":"#27AE60","CRITIQUE":"#E67E22","COUPE":"#C0392B"}.get(pred_mode_val, "#95A5A6")

            p1, p2, p3 = st.columns(3)
            p1.metric("Tension prédite", f"{pred.get('pred_v','—')} V" if pred.get("pred_v") else "—")
            p2.metric("Inférences",      str(pred.get("inferences", 0)))
            p3.metric("Statut",          "Prêt" if pred.get("ready") else "Apprentissage")

            thr_parts = ""
            if pred.get("thr_normal") is not None:
                thr_parts += f'&nbsp;&nbsp;<span style="font-size:12px;opacity:0.7">| Seuil NORMAL adapté : <strong>{pred["thr_normal"]} %</strong></span>'
            if pred.get("thr_critique") is not None:
                thr_parts += f'&nbsp;&nbsp;<span style="font-size:12px;opacity:0.7">| Seuil CRITIQUE adapté : <strong>{pred["thr_critique"]} %</strong></span>'

            st.markdown(
                f'<div class="pred-box">'
                f'<span style="font-size:11px;opacity:0.6;letter-spacing:1px">MODE PRÉDIT</span><br>'
                f'<span style="font-size:28px;font-weight:900;color:{pred_color}">{pred_mode_val}</span>'
                f'{thr_parts}</div>',
                unsafe_allow_html=True
            )
=======
...
<<<<<<< HEAD
            ...
            if mc3.button("🔴 COUPE",    use_container_width=True): send_cmd({"cmd":"SET_MODE","mode":2}); st.rerun()
...
<<<<<<< HEAD
            ...
            if st.button("Appliquer les seuils", use_container_width=True):
                if sn > sc_val > 0:
                    send_cmd({"cmd":"SET_THRESHOLDS","normal":sn,"critique":sc_val})
                    st.rerun()
                else:
...
<<<<<<< HEAD
            ...
            rows = {
                "Capteur":["INA219 (V+I)","DHT22 (T+H)","BH1750 (lux)","SD Card","WiFi ESP32"],
                "Statut":[
                    "🟢 OK",
                    "🟢 OK" if dht_ok else "🔴 Erreur",
...
<<<<<<< HEAD
        ...
        if last and last.get("predictif"):
            ...
            st.table({
                ...
                "Valeur":[
                    "Prêt" if pred.get("ready") else "En apprentissage",
                    str(pred.get("inferences", 0)),
                    f"{pred.get('pred_v','—')} V" if pred.get("pred_v",0) > 0 else "—",
                    pred.get("pred_mode","—")
                ]
            })
            if pred.get("thr_normal") is not None:
                st.caption(f"Seuil NORMAL adapté : {pred['thr_normal']} %")
            if pred.get("thr_critique") is not None:
